...
{
public:
	sphere(vec3d pos, mat3d inertia=Eigen::MatrixXd::Identity(3,3), real_t radius =5, real_t mass = 1, vec3d vel = vec3d(0, 0, 0), vec3d rot=vec3d(0,0,0), vec3d angvel=vec3d(0,0,0), quaternion_t quat = quaternion_t(1, 0, 0, 0)) :rigidbody(pos, inertia, mass, vel,rot, angvel, quat)
	{
	...
	void addtoscene(ISceneManager* smgr, IVideoDriver* driver)const override
	{
		...
		Node->setRotation(vector3df(rot_.x(), rot_.y(), rot_.z()));
